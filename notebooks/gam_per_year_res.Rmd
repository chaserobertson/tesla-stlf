...
model_order <- c("gamm", "rf.res.gamm", "gamm.rad",
                 "rf.res.gamm.rad", "gamm.all", "rf")

results |> 
    pivot_longer(cols = c("rmse", "mape", "mae"), names_to = "metric") |>
    mutate(metric = toupper(metric), model = fct_relevel(model, model_order)) |>
ggplot(aes(x = train_begin, y = value, colour = model, group=model)) + 
    facet_wrap(~ metric, ncol = 1, scales = "free_y") +
    geom_point() + geom_line() +
    scale_colour_brewer(type='qual', palette='Paired') +
    theme_minimal()
ggsave(paste0('plots/gamm_residual_', region, '.png'), width=6, height=5)
```
