...
fit.gamm <- function(start, form) {
    ...
    test <- subset(fitdata[end:(end + 24*7),], wday %in% 1:5)
    
    wtdyear <- mgcv::gamm(form,  data = train)
    return(mape(test$Demand, predict(wtdyear$gam, test)))
}
...
results |> 
    pivot_longer(cols=c("gamm", "gamm.rad", "gamm.all"), values_to = "mape") |>
    mutate(name = fct_reorder2(name, train_begin, mape)) |>
    ggplot(aes(train_begin, mape, colour=name)) + 
    geom_line() + geom_point() +
    scale_colour_viridis_d() +
    theme_minimal() + theme(aspect.ratio=1/3)
#ggsave(paste0('plots/gamm_', region, '.png'), width=9, height=3)
```

```{r}
colMeans(results[3:5])
```
