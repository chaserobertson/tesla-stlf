...
gamlwmod <- Demand ~ s(DSTTime, bs = "cc") + s(WtdTemp, bs = "tp") + s(Year, bs = "tp")
rad.form <- Demand ~ s(DSTTime, bs = "cc") + s(WtdTemp, bs = "tp") + s(Year, bs = "tp") + 
    s(direct_normal_irradiance..W.m.., bs="tp")
all.form <- Demand ~ s(DSTTime, bs = "cc") + s(WtdTemp, bs = "tp") + s(Year, bs = "tp") + 
    s(direct_normal_irradiance..W.m.., bs="tp") + #s(direct_radiation..W.m.., bs="tp") + 
    s(shortwave_radiation..W.m.., bs="tp") + s(windspeed_10m..km.h., bs="tp") + 
    s(winddirection_10m....) + s(rain..mm.) + s(diffuse_radiation..W.m..) +
    s(cloudcover....) + s(relativehumidity_2m....)
...
n_splits <- 22
start <- head(seq(1, nrow(fitdata), by = as.integer(nrow(fitdata) / n_splits)), -2)
...
results |> 
    ...
    ggplot(aes(train_begin, r.sq, colour=name)) + 
    geom_line() + geom_point() +
    scale_colour_viridis_d() +
    theme_minimal() + theme(aspect.ratio=1/3)
ggsave(paste0('plots/gamm_r2_', region, '.png'))
```
