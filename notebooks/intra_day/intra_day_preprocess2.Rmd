...
```{r}
df <- read.csv("data/sa/merged_interpolated.csv")
...
Create standardised time of the Year with range [0,1), 0 being first observation of the year and 1 being just after the final observation (by standard time). Also add time-weighted temperature.
...
Remove weekends. Resulting diff should match either 1/n_obs or a post-weekend step of 48*2/n_obs.
...
```{r}
df.week <- df.week[,c("ACDT", "Demand", "Time", "DSTTime", "Year", "Temp", "WtdTemp")]
head(df.week)
```
...
Write each complete year of weekdays to file.
...
Also write complete data with new fields to file.

```{r}
write.csv(df, "data/intra_day/SA_preprocessed.csv", row.names=F)
```
